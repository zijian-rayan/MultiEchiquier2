...
BEGIN_MESSAGE_MAP(CMultiEchiquierApp, CWinAppEx)
	...
	ON_COMMAND(ID_FICHIER_OPENIMAGE, &CMultiEchiquierApp::OnOpenImage)
END_MESSAGE_MAP()
...
void CMultiEchiquierApp::OnOpenImage()
{
  CFileDialog cfd(TRUE,_T("bmp"),0,OFN_FILEMUSTEXIST|OFN_HIDEREADONLY|OFN_PATHMUSTEXIST,
    _T("Bitmap Files (*.bmp)|*.bmp|Dib Files (*.dib)|*.dib||"));
  if(cfd.DoModal()==IDOK)
  {
    CMainFrame* pFrame = STATIC_DOWNCAST(CMainFrame, m_pMainWnd);
    CChildFrame*wnd=(CChildFrame*)pFrame->CreateNewChild(RUNTIME_CLASS(CChildFrame),IDR_MultiEchiquierTYPE,m_hMDIMenu,m_hMDIAccel);
    wnd->m_wndView.LoadImage(cfd.GetPathName());
  }
}
